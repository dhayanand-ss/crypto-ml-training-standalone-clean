...
from ..artifact_control.gcs_manager import GCSManager
...
def log_metrics(model, loader, device="cuda", name="val", step=None):
    labels, preds = evaluate_model(model, loader, device=device)
...
def main(args):
    ...
    try:
        ...
        print(f"WARNING: Failed to load latest model from MLflow: {e}")
    ...
    with mlflow.start_run() as run:
        ...
        for epoch in range(epochs):
            ...
            log_metrics(model, val_loader, device=device, name="val", step=epoch)
        ...
        test_report = log_metrics(model, test_loader, device=device, name="test")
        ...
        log_metrics(model, dataloader, device=device, name="past_perf")
        pred = []
        model.eval()
        with torch.no_grad():
        ...
        try:
            ...
            print(f"WARNING: Failed to save/register model to MLflow: {e}")
